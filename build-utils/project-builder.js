...
export async function buildProject(projectName, templateType, outputPath) {
  ...
  // 5. 템플릿에 컴포넌트 내용 주입
  templateContent = templateContent.replace(/<style id="style-container">[\s\S]*?<\/style>/, `<style id="style-container">\n${combinedCSS}\n</style>`);

  templateContent = templateContent.replace(/<div id="contents">[\s\S]*?<\/div>/, `<div id="contents">\n${componentHTMLs.join('\n')}\n</div>`);

  templateContent = templateContent.replace(/<script id="script-container">[\s\S]*?<\/script>/, `<script id="script-container">\n${combinedJS}\n</script>`);
...
export function copyMediaFiles(projectName, outputPath) {
  ...
  const projectSrcDir = path.join(rootDir, 'src', projectName);
  const imagesDir = path.join(projectSrcDir, 'assets/images');
  const videosDir = path.join(projectSrcDir, 'assets/videos');
